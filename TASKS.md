...
- [x] Make puzzle generation deterministic in tests by injecting RNG instead of relying on `Math.random()` throughout puzzle classes and `PuzzleFactory`. (completed 2026-03-06: added RNG injection via `BasePuzzle` + `PuzzleFactory` and deterministic sequence-based tests in `src/puzzles/*.test.ts`)
